 ...
 * belongs to.
 */
public interface FederationSubClusterResolver extends Configurable {

  /**
   * Obtain the sub-cluster that a specified node belongs to.
   * 
   * @param nodename the node whose sub-cluster is to be determined
   * @return the sub-cluster as identified by the {@link FederationSubClusterId}
   ...
   * @return the sub-clusters as identified by the
   *         {@link FederationSubClusterId} that have nodes on the given rack
